...
function buscarPeliculas(peliculas, titulo = '', genero = '') {
    if (!titulo && !genero) {
        return peliculas;
    }
    ...
    if (titulo) {
        resultado = resultado.filter(pelicula => pelicula.titulo.includes(titulo));
    }

    if (genero) {
        resultado = resultado.filter(pelicula => pelicula.genero === genero);
    }
